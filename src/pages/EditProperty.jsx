...
export default function EditProperty({}) {
    ...
    const fetchPropertyDetails = async () => {
      try {
        ...
        const propertyRecord = {
          ...
          kabja_dinank: formatDateToYYYYMMDD(data.propertyRecords[0].kabja_dinank),
        };
        const propertyRecordDetail = {
          ...data.propertyRecordDetail,
          panjikaran_dinank: formatDateToYYYYMMDD(data.propertyRecordDetail.panjikaran_dinank),
          avantan_dinank: formatDateToYYYYMMDD(data.propertyRecordDetail.avantan_dinank),
          ...
          bhavan_nirman: data.propertyRecordDetail.bhavan_nirman === "Yes" ? "true" : "false",
          first_installment_due_date: formatDateToYYYYMMDD(data.propertyRecordDetail.first_installment_due_date),
        };
        ...
        const serviceCharges = data.serviceCharges.map((charge) => ({
          ...charge,
          service_charge_payment_date: formatDateToYYYYMMDD(charge.service_charge_payment_date),
        }));
        setFormData({
          propertyRecord,
          propertyRecordDetail,
  ...
  // Calculate first_installment_due_date
  useEffect(() => {
  ...
  // Calculate avshesh_dhanrashi
  useEffect(() => {
    const auctionKeemat = parseFloat(formData.propertyRecordDetail.auction_keemat || "0");
    const panjikaranDhanrashi = parseFloat(formData.propertyRecordDetail.panjikaran_dhanrashi || "0");
    const avantanDhanrashi = parseFloat(formData.propertyRecordDetail.avantan_dhanrashi || "0");
    const avsheshVikrayMulyaEkmusht = parseFloat(
      formData.propertyRecordDetail.avshesh_vikray_mulya_ekmusht_jama_dhanrashi || "0"
    );
    const ekmusht_jama_dhanrashi = parseFloat(formData.propertyRecordDetail.ekmusht_jama_dhanrashi || "0");
    const computedAvshesh =
      auctionKeemat -
      (panjikaranDhanrashi + avantanDhanrashi + avsheshVikrayMulyaEkmusht + ekmusht_jama_dhanrashi);
    setFormData((prev: any) => ({
      ...prev,
      propertyRecordDetail: {
        ...prev.propertyRecordDetail,
        avshesh_dhanrashi: computedAvshesh > 0 ? computedAvshesh.toFixed(2) : "0.00",
      },
    }));
  }, [
  ...
  // Prefill installment amounts
  useEffect(() => {
    if (currentStep === 4 && !currentPaymentEntry.installment_amount) {
      const avsheshDhanrashi = parseFloat(formData.propertyRecordDetail.avshesh_dhanrashi || "0");
      const interestRate = parseFloat(formData.propertyRecordDetail.interest_rate || "0");
      const timePeriod = parseFloat(formData.propertyRecordDetail.time_period || "0");
  ...
  }, [currentStep, formData.propertyRecordDetail, currentPaymentEntry.installment_amount]);

  // Calculate late fee
  useEffect(() => {
    if (currentStep === 4 && formData.propertyRecordDetail.first_installment_due_date) {
      const installmentNumber = formData.installments.length + 1;
      const dueDate = addMonths(formData.propertyRecordDetail.first_installment_due_date, (installmentNumber - 1) * 3);
  ...
  const handleInputChange = (section: string, fieldId: string, value: any) => {
    // Special handling for kabja_dinank
    if (fieldId === "kabja_dinank") {
      const formattedDate = value ? formatDateToYYYYMMDD(value) : null;
      setFormData((prev: any) => ({
        ...prev,
        [section]: {
          ...prev[section],
      ...
      if (!formattedDate && value) {
        setErrors((prev) => ({
          ...prev,
          kabja_dinank: "Invalid date format. Use DD-MM-YYYY.",
        }));
      } else {
        setErrors((prev) => {
          const newErrors = { ...prev };
          delete newErrors.kabja_dinank;
          return newErrors;
        });
      }
      return;
    ...
    // Special handling for avshesh_vikray_mulya_ekmusht_jama_dinank
    if (fieldId === "avshesh_vikray_mulya_ekmusht_jama_dinank") {
      const formattedDate = value ? formatDateToYYYYMMDD(value) : null;
      setFormData((prev: any) => ({
        ...prev,
        [section]: {
      ...
      if (!formattedDate && value) {
        setErrors((prev) => ({
          ...prev,
          avshesh_vikray_mulya_ekmusht_jama_dinank: "Invalid date format. Use DD-MM-YYYY.",
        }));
      } else {
        setErrors((prev) => {
          const newErrors = { ...prev };
          delete newErrors.avshesh_vikray_mulya_ekmusht_jama_dinank;
          return newErrors;
        });
      }
      return;
    ...
    // Special handling for bhavan_nirman
    if (fieldId === "bhavan_nirman") {
      setFormData((prev: any) => ({
        ...prev,
        [section]: {
          ...prev[section],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmSubmission = window.confirm("क्या आप संपत्ति विवरण जमा करने के लिए निश्चित हैं?");
    if (!confirmSubmission) return;
    setIsSubmitting(true);
    try {
      const pr = formData.propertyRecord;
      const prd = formData.propertyRecordDetail;
      const bhavanNirmanValue = prd.bhavan_nirman === "true" || prd.bhavan_nirman === true ? "Yes" : "No";
      const payload = {
        propertyRecords: [
          {
            id: pr.id,
            ...
            kabja_dinank: formatDateToDDMMYYYY(pr.kabja_dinank),
        ...
        propertyRecordDetail: {
          ...
          patta_bhilekh_dinank: formatDateToDDMMYYYY(prd.patta_bhilekh_dinank) || null,
          bhavan_manchitra_swikrit_manchitra: prd.bhavan_manchitra_swikrit_manchitra || null,
          bhavan_nirman: bhavanNirmanValue,
          jama_dhan_rashi_dinank: formatDateToDDMMYYYY(prd.jama_dhan_rashi_dinank) || null,
          ...
          first_installment_due_date: formatDateToDDMMYYYY(prd.first_installment_due_date),
        },
        installments: formData.installments
          .filter((payment: any) => !payment.payment_id)
          .map((payment: any) => {
            const payAmount =
        ...
        serviceCharges: formData.serviceCharges
          .filter((charge: any) => !charge.service_charge_id)
          .map((charge: any) => ({
            property_id: pr.property_id,
            property_record_id: prd.id,
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="mt-6">
            <div className="max-h-[calc(100vh-320px)] overflow-y-auto px-2 custom-scrollbar">
                    ...
                    return (
                      <div key={field.id} className="mb-4">
                        ...
                        {field.readOnly ? (
                          ...
                          />
                        ) : field.type === "boolean" || field.id === "bhavan_nirman" ? (
                          <select
                            className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-blue-500 dark:focus:border-blue-400 focus:ring-1 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors text-sm shadow-sm"
                            onChange={(e) =>
                              handleInputChange(sectionMap[currentStep], field.id, e.target.value)
                          ...
                          />
                        ) : field.type === "date" || field.id === "kabja_dinank" || field.id === "avshesh_vikray_mulya_ekmusht_jama_dinank" ? (
                          <input
                            type="date"
                            className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-blue-500 dark:focus:border-blue-400 focus:ring-1 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors text-sm shadow-sm"
                            value={fieldValue}
              ...
              {currentStep === 4 && formData.propertyRecordDetail.first_installment_due_date && (
                <div className="mb-4">
                  ...
                  {formData.installments.length > 0 && (
                    <div className="mt-6 overflow-x-auto">
                      <table className="min-w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
                        ...
                        <tbody>
                          {formData.installments.map((payment: any, index: number) => {
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                          <p className="text-sm text-gray-600 dark:text-gray-400">ब्याज राशि (छह महीने)</p>
                          <p className="text-lg font-semibold text-gray-900 dark:text-white">
                        ...
                        <div>
                          <p className="text-sm text-gray-600 dark:text-gray-400">तिमाही किस्त धनराशी</p>
                          <p className="text-lg font-semibold text-gray-900 dark:text-white">
                        ...
                        <div>
                          <p className="text-sm text-gray-600 dark:text-gray-400">प्रति दिन विलंब धनराशि</p>
                          <p className="text-lg font-semibold text-gray-900 dark:text-white">
                            {formatIndianNumber(lateFeePerDay)}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600 dark:text-gray-400">प्रथम किश्त नियत तिथि</p>
                          <p className="text-lg font-semibold text-gray-900 dark:text-white">
                            {formatDateToDDMMYYYY(formData.propertyRecordDetail.first_installment_due_date)}
                          </p>
                        </div>
                      </div>
                    );
